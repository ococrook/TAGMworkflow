...
become increasingly popular because of its ability to now systematically localise 
thousands of proteins per experiment. In parallel with these experimental advances,
there has has also been improvements on the analysis of spatial proteomics data.
In this workflow, we demonstate using `pRoloc` for the Bayesian analysis of 
...
assessing convergence, and interpretting downstream results. In several places
we provide additional details on Bayesian analysis to provide users with a
holisitic view of Bayesian analysis for spatial proteomics data. 
...
As a result of these spatial protomics technolgies many biological systems have 
...
take the processed output and explain how to intepret the results, as 
...
MCMC method, we run a brief analysis on a subsest (400 randomly
...
the TAGM model. In the code chunk below, we load a pre-computated
...
The following code chunks set up a manual convegence diagnostic
...
set the `transform` argument to apply log tranformation.  @Gelman:1992
...
have low uncertainty in their localisation, whlist those with higher Shannon
entropy have uncertain localisation. Since multi-localised protein have
uncertain localisation to a single sub-celullar niche, exploring the Shannon
