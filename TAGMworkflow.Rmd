...
of a generative model for the data. In a Bayesian framework, a model
...
This approach uses the expectation-maximisation (EM) algorithm to perform inference.
...
and the current Bioconductor version of `pRolocdata` and `MSnbase`. 
...
quality control, supervised machine learning and transfer learning see Lisa's workflow.
We use a spatial proteomics dataset on Mouse E14TG2a embryonic stem cells. 
The LOPIT protocol was used and normalised intensity of proteins from eight iTRAQ 8-plex
...
such as PCP, and when fractionation is perform by differential centrifugation.
...
observe that some of the organlles classes overlap and this is a typical 
...
does not change. This fact can be used to assess the convergence of the EM
...
and this should not be concerned users.
...
In our particular case, we use a so-called collapsed Gibbs sampler.
...
a probability distribution, in our case the posterior distribution of the parameters.
...
convergence obtain samples form the posterior distribution. A specific
...
a method to reduce posterior variance, to obtain a collapsed Gibbs sampler.
...
and diagnosing convergence is an active area of research and throughout the `90s many approaches
...
We first assess converged using a parallel chains approach. Producing multiple
...
Thus far, our analysis looks very good. Each chain oscillate around an
...
chains and so we recalculate the diagnostic excluding these chains. The computed gelman
...
and $4$. An $\hat(R)$ statistics of $1$ is indicative of convergence
...
Along with the Gelman diagnostics, which use parallel chains, 
...
of iterations are significantly different from the the mean calculated from  last $50\%$ of iterations.
If they are significantly different (at say a level 0.01) then this is evidence that particular
...
these chains have not conveged. Users can use this method to explore
other outputs should they wish. 

An important question at this point is if removing
early portion of the chain might lead to improvment of the convergence diagonistics.
This my particularly relevant if a chain converges some iterations after our
`burin` specified orginally. For example let us take the first Geweke test
