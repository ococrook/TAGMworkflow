...
Thus far, our analysis appears promising. Each chain oscillate around an
...
For a more quantitaive analysis, we again apply the Gelamn diagnostics to these summaries.
```{r,}
...
from any furthe analysis. The code chunk below remove these chains and
creates and new object to store the converged chains.

```{r,}
removeChain <- c(3, 5, 6) # The chain removed
tagmE14_converged <- tagmE14[seq_len(nChains)[-removeChain]] # Create new object

```

We have seen that the `MCMCParams` object can be large and therefore
...
evaluate demonstrates, retaining every $5^{th}$ iteration
