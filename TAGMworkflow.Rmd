...
\@ref(fig:probdists), we can obtain the full posterior distribution
...
```{r probdists, fig.height = 8, fig.with = 6, fig.cap = "Full posterior distribution of localisation probabilities for individual proteins.", warning = FALSE}
...
The Bayesian analysis of biological data is of clear interest to many because
of its ability to provide richer information about the experimental results.
A fully Bayesian analysis differs from other machine learning approaches, since 
it can quantify the uncertainty in our experiments. Furthermore, a generative model
is used to explicitly describe the data rendering the output more interpretable,
rather than the less intepretable outputs of black-box classifiers.
