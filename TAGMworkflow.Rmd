...
with some remarks and direction for the future.
...
For information on how to import data, perform basic data processing,
...
We can visualise the mouse stem cell dataset use the `plot2D` function. As
we can see some of the classes overlap - it is there vital to perform
uncertainty quantification when analysising this data.
...
they can be changed, which we explain later. The output is an object of class `MAPParams`.
...
In practice, it is not unexpected to observe small fluctations due to numerical errors
and users should not be concerned by this.
