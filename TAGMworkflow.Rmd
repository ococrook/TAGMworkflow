...
were proposed, (see ...). A converged MCMC algorithm should be oscillating rapidly around
...
choice recommended by . The prior mean `mu0` is set to mean of the data.
...
$2$, which is the smallest integer value that ensure a finite covariance matrix.
The prior scale matrix $S0$ is set to
\begin{equation}
S0 = \frac{\diag(\frac{1}{n}\sum (X - \bar{X})^2)}{K^{1/D}},
\end{equation}
...
for the Dirichlet disctribution concentration paramters `\beta0` is set to $1$
...
Jeffery's prior for the dirichlet, which sets `\beta0` is $0.5$ for each organelle.
